...
The sample will keep the existing default server-loaded sample path for first load. When the user selects a local file, the sample will:
...
9. The original default `../data/sample.las` flow still works.
...
14. The HTTPS development server returns the default `data/sample.las`.
...
- The existing default `/data/sample.las` loading path still exists through `loadLasPoints(samplePath, ...)`.
